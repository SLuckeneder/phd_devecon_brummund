...
rm(list = ls())
setwd("replication_data_FRJ2015") #Set working directory to wherever data files are saved
load("AEJApp-2014-0201_Dataset/devDTA.Rdata")
names(devDTA)
...
for (i in 1:3) {
  ...
  if (i==1) {
    Match<-Match(Y=Plit, Tr=Tr, X=X, estimand="ATT", exact=c(TRUE, TRUE, TRUE, FALSE), replace=FALSE)
  } else if (i==2) {
    Matched_norep<-Match(Y=Plit, Tr=Tr, X=X, estimand="ATT", exact=c(TRUE, TRUE, TRUE, FALSE), replace=FALSE, caliper=c(0,0,0,.5))
  } else if (i==3) {
    library(cem)
    # Matched_cem <- cem::cem(treatment = "Tr", data = data.frame(X, Tr))
    drop_but <- function(x, but) {names(x)[!grepl(paste0(but, collapse = "|"), names(x))]}
    match_on <- c("State_no_2001_old", "DELIM_district_no", "PC_no_1976", "SC_percent71_true")
    Matched_cem <- cem(treatment = "AC_type_noST", data = matchdta,
        drop = drop_but(matchdta, match_on),
        keep.all = TRUE)
  ...
  ##output
  if(i==1){
    treatedDTA<-matchdta[Match$index.treated,]
    controlDTA<-matchdta[Match$index.control,]
    ...
    mDTA <- matchdta[Matched_cem$w > 0,]
    treatedDTA<-mDTA[mDTA$AC_type_noST == "SC",]
    controlDTA<-mDTA[mDTA$AC_type_noST == "GEN",]
    treatedDTA$index.match<-c(1:dim(treatedDTA)[1])
    controlDTA$index.match<-c(1:dim(controlDTA)[1])
    matched3<-rbind(treatedDTA, controlDTA)	
  }
...
for(i in 2:length(outcomeindex)){
  ...
  #And now with weighted cem
  mymodel<-lm(data = matchdta, formula = paste0(colnames(matchdta)[outcomeindex[i]], "~ AC_type_noST"), weights = Matched_cem[["w"]])
  
  #SEs clustered at state level
  mySE<-clusterSE(mymodel, data= matchdta, cluster="State_no_2001_old")
...
for (i in 1:nrow(figurematrix)) {
  arrows(x0=as.numeric(figurematrix[i,2]), x1= as.numeric(figurematrix[i,3]), y0=(15-i+.15), y1=(15-i+.15), angle=90, length=.025, code=3)
  
  arrows(x0=as.numeric(figurematrix[i,6]), x1= as.numeric(figurematrix[i,7]), y0=(15-i), y1=(15-i), angle=90, length=.025, code=3)
  
  arrows(x0=as.numeric(figurematrix[i,10]), x1= as.numeric(figurematrix[i,11]), y0=(15-i-.15), y1=(15-i-.15), angle=90, length=.025, code=3)
  
  points(x=as.numeric(figurematrix[i,1]), y=15-i+.15, pch=19, cex=.7)
  
  points(x=as.numeric(figurematrix[i,5]), y=15-i, pch=1, cex=.7 ,  col="gray40")
  
  points(x=as.numeric(figurematrix[i,9]), y=15-i-.15, pch=6, cex=.7 ,  col="gray40")
  
  text(x=95, y=i, figurematrix[i,4], cex=.8)
  
}
legend(x=2.2, y=4, pch=c(19, 1, 6, NA), lty=c(NA, NA, NA, 1), legend=c("Orig. matching est.", "CEM est.", "CEM weighted est.", "95% conf. interval"), cex=.9, bg="white", merge=T)
dev.off()
